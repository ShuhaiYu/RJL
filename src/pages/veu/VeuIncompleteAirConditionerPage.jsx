// src/pages/veu/VeuIncompleteAirConditionerPage.jsx
...
export default function VeuIncompleteAirConditionerPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${baseApi}/veu-projects?is_completed=false&type=air_conditioner`,
      ...
      const msg =
        err.response?.data?.message || "Failed to fetch Air Conditioner list";
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center justify-center w-12 h-12 bg-primary-100 rounded-lg">
            <KeenIcon icon="air" className="text-2xl text-primary-600" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Incomplete Air Conditioner
              {agencyNameFromState ? ` - ${agencyNameFromState}` : ""}
            </h1>
            <p className="text-gray-600">
              Properties with incomplete air conditioner
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Stats */}
      {/* <StatsCards
        title="Air Conditioner Statistics"
        loading={loading}
        cards={[
          {
            key: "total",
